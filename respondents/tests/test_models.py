...
def test_get_lender_hierarchy():
    ...
    institution = Institution.objects.filter(
        institution_id="11000000002").first()
    ...
    """
    institution = Institution.objects.filter(
        institution_id="91000000003").first()
    hierarchy_list = institution.get_lender_hierarchy(False, False, 2013)
    assert len(hierarchy_list) == 1
    assert hierarchy_list[0].institution_id == "91000000003"
    ...
    institution = Institution.objects.filter(
        institution_id="91000000001").first()
    hierarchy_list = institution.get_lender_hierarchy(False, False, 2013)
    ...
    assert hierarchy_list_order[0].institution_id == "91000000001"
    hierarchy_list_exclude_order = institution.get_lender_hierarchy(
        True, True, 2013)
    assert hierarchy_list_exclude_order[0].institution_id == "91000000002"
