...
class ViewsTest(TestCase):
    ...
    def test_race_summary(self):
        resp = self.client.get(
            reverse('censusdata:race_summary'),
            {
        ...
        self.assertEqual(
            resp['20131122233300']['non_hisp_white_only_perc'], .2)
        self.assertEqual(
            resp['20131122233300']['non_hisp_black_only_perc'], .4)
        self.assertEqual(
            resp['20131122233300']['non_hisp_asian_only_perc'], .5)
        ...
        self.assertEqual(
            resp['20131122233400']['non_hisp_white_only_perc'], .05)
        self.assertEqual(
            resp['20131122233400']['non_hisp_black_only_perc'], .25)
        self.assertEqual(
            resp['20131122233400']['non_hisp_asian_only_perc'], .2)

    def test_race_summary_csv(self):
        resp = self.client.get(reverse('censusdata:race_summary_csv'))
        self.assertEqual(resp.status_code, 404)
        resp = self.client.get(
            reverse('censusdata:race_summary_csv'),
            {
                'action_taken': '1,2,3,4',
