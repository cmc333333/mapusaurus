...
def tables(request):
    ...
    return HttpResponse(
        json.dumps(table_data), content_type='application/json')
...
def tables_csv(request):
    ...
    file_name = 'HMDA-Summary-Table_Year%s_Lender%s_MSA%s.csv' % (
...
def hmda(request):
    ...
    return HttpResponse(
        json.dumps(loan_originations_as_json(request)),
        content_type='application/json',
    )


def census(request):
    """This endpoint returns census data used for circle coloring over
    tracts"""
    return HttpResponse(
