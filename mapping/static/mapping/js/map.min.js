...
function initCalls(geoQueryType) {
    ...
    "selected" === gt ? (blockStuff(), $.when(getTractData(action, !1)).done(function(data1) {
        init(data1);
    })) : "all" === gt && (blockStuff(), $.when(getTractData(action, getBoundParams())).done(function(data1) {
        init(data1);
    }));
}
...
function getTractData(actionTakenVal, bounds) {
    ...
    params.swLat = bounds.swLat, params.swLon = bounds.swLon) : urlParam("metro") ? params.metro = urlParam("metro") : urlParam("county") ? params.county = urlParam("county") : console.log("No metro or bounds provided");
    var hash = getHashParams();
    return void 0 !== hash.lh && (params.lh = hash.lh.values), void 0 !== hash.peers && (params.peers = hash.peers.values), 
...
function drawCircle(geo, options) {
    ...
    circle.tractid = geo.tractid || "", circle.perThousandHouseholds = geo.per_thousand_households, 
    circle.type = "tract-circle", circle.keyCircle = 0, void 0 !== options && (circle.keyCircle = options.keyCircle), 
    circle.on("mouseover mousemove", function(e) {
        new L.Rrose({
            ...
            autoPan: !1,
            y_bound: 160
        }).setContent('<div class="bubble-header">Tract ' + circle.tractid + '</div><div class="lar-count"><span class="circle-hover-data">' + data.volume + '</span><span class="circle-hover-label">LAR</span></div><div class="hh-count"><span class="circle-hover-data">' + data.num_households + '</span><span class="circle-hover-label">Households</span></div></div>').setLatLng(e.latlng).openOn(map);
...
function buildKeyCircles() {
    ...
    for (var posx = 0, rad = 0, maxRad = circles[circles.length - 1]._radius, posy = 2 * maxRad, textPosy = posy + 16, svgStr = '<svg height="' + (2 * maxRad + 20) + '">', i = 0; i < circles.length; i++) {
...
function getRange(data) {
    var circles = _.filter(data, function(circle) {
        return "tract-circle" === circle.type && circle.perThousandHouseholds > 0;
    });
    if (circles = _.sortBy(circles, function(circle) {
        return circle.perThousandHouseholds;
    }), circles.length <= 3) return circles;
...
function setMapHeight() {
    var viewportHeight = $(window).height(), headerHeight = $("#header").outerHeight(), mapHeaderHeight = $("#map-header").outerHeight(), combinedHeadersHeight = headerHeight + mapHeaderHeight, mapHeight = viewportHeight - combinedHeadersHeight;
...
function blockStuff() {
    if (!0 === isUIBlocked) return !1;
    isUIBlocked = !0, $.blockUI({
        css: {
            border: "none",
            padding: "15px",
            backgroundColor: "#000",
...
function hmdaStat(tractData) {
    var mediumCircleSize = 5e4, area = mediumCircleSize * tractData.per_thousand_households / window.avgPerThousandHouseholds;
    return Math.sqrt(area) || 0;
}

function layerUpdate(layer) {
    var layerEval, mbLayer, keyPath;
    ...
    layerEval = getLayerType(layer), mbLayer = layerEval.layer, layerEval.type, keyPath = layerEval.keyPath, 
    keyPath ? ($("#altScaleImg").attr("src", keyPath), $("#altScaleImgLabel").text(layerEval.displayName), 
...
function urlParam(field) {
    var url = decodeURIComponent(window.location.search.substr(1)), keyValueStrs = url.split("&"), pairs = _.map(keyValueStrs, function(keyValueStr) {
        return keyValueStr.split("=");
    });
    return _.reduce(pairs, function(soFar, pair) {
...
function getPeerLinks() {
    ...
    $.each(peerList, function(i, val) {
        var instid = $(val).data("instid"), metroParam = "&metro=" + (urlParam("metro") || ""), countyParam = "&country=" + (urlParam("county") || ""), href = "/map/?lender=" + instid + "&year=" + selectedYear + metroParam + countyParam + window.location.hash.replace("&peers=true", "");
        $(val).attr("href", href);
    });
}

function updateCensusLink() {
    var actions = getHashParams(), actionVar = getActionTaken(actions.action.values), metroParam = "&metro=" + (urlParam("metro") || ""), countyParam = "&country=" + (urlParam("county") || "");
...
$(document).ready(function() {
    ...
    }), void 0 !== loadParams.action ? $("#action-taken-selector").val(loadParams.action.values) : addParam("action", "all-apps-5"), 
    void 0 !== loadParams.lh ? (lhStatus = "true" === loadParams.lh.values, $("#superSelect").prop("checked", lhStatus), 
    $("#peerSelect").prop("disabled", lhStatus), toggleSuper(lhStatus)) : addParam("lh", !1), 
    ...
    }), void 0 !== loadParams.geo_query_type ? (geoQueryType = loadParams.geo_query_type.values, 
    ...
    }), void 0 !== loadParams.branches ? (branchStatus = "true" === loadParams.branches.values, 
    $("#branchSelect").prop("checked", branchStatus), toggleBranches(branchStatus)) : addParam("branches", !1), 
    $("#branchSelect").change(function() {
        toggleBranches($("#branchSelect").prop("checked"));
    }), void 0 !== loadParams.peers ? !0 === lhStatus ? ($("#peerSelect").prop("checked", !1), 
    ...
    }), void 0 !== loadParams.category ? (assignCat(loadParams.category.values), layerUpdate(cat), 
    $(catId).addClass("active-layer")) : (assignCat("layer_pct_minority"), layerUpdate(cat), 
...
var moveEndAction = {}, oldEndAction = geoQueryType;
...
var rawGeo, rawLar, rawMinority, rawData, selectedYear = selectedYear || 2015, isUIBlocked = !1, larVolume = [], pctMinority = [], dataStore = {};
