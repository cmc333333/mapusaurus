...
class HMDARecord(models.Model):
    ...
    msamd = models.CharField(
        max_length=5,
        help_text=("A code representing the Metropolitan Statistical "
                   "Area/Metropolitian Division (MSA/MD) the property is "
                   "located in. An MSA is a region with relatively high "
                   "population density at its core (usually a single large "
                   "city) and close economic ties throughout. Larger MSAs "
                   "are divided into MDs."),
    )
    ...
    census_tract_number = models.CharField(
        max_length=7,
    ...
    population = models.CharField(
        max_length=8, help_text="The total population in the tract.")
    minority_population = models.CharField(
        max_length=6,
        help_text=("The percentage of minority population to total "
                   "population for the census tract, carried to two decimal "
                   "places."),
    )
    ffieic_median_family_income = models.CharField(
        max_length=8,
        help_text=("The median family income in dollars for the MSA/MD in "
                   "which the tract is located. (Note: The HMDA API names "
                   "this col hud_median_family_income.)"),
    )
    tract_to_msamd_income = models.CharField(
        max_length=6,
        help_text=("The percentage of the median family income for the tract "
                   "compared to the median family income for the MSA/MD, "
                   "rounded to two decimal places."),
    )
    number_of_owner_occupied_units = models.CharField(
        max_length=8,
        help_text=("The number of dwellings in the tract that are lived in "
                   "by the owner."),
    )
    number_of_1_to_4_family_units = models.CharField(
        max_length=8,
        help_text=("The number of dwellings in the tract that are built to "
                   "house fewer than 5 families."),
    )
    ...
    institution = models.ForeignKey(
        'respondents.Institution', to_field='institution_id')
    geo = models.ForeignKey('geo.Geo', to_field='geoid', db_index=True)
...
class Year(models.Model):
    ...
    hmda_year = models.PositiveIntegerField(
        primary_key=True,
        choices=YEAR_CHOICES,
        help_text="The reporting year of the HMDA record.",
    )
    census_year = models.PositiveIntegerField(
        choices=YEAR_CHOICES, help_text="Year of census data.", null=False)
    geo_year = models.PositiveIntegerField(
        choices=YEAR_CHOICES,
        help_text="Year that geographic boundaries were recorded.",
        null=False,
    )
...
class LendingStats(models.Model):
    ...
    fha_count = models.PositiveIntegerField(
        help_text="Total HMDA LAR count where loan_type=2(FHA)")
    fha_bucket = models.PositiveIntegerField(
        help_text=("Predetermined buckets calculated by find fha % of total "
                   "lar count"),
    )
